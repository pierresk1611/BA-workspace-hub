...
const PRIORITY_COLORS: Record<string, string> = {
  ...
  'Nízka': 'text-emerald-600 bg-amber-50 border-amber-200', // Fixed color
};
...
export function AllProjectsDashboard() {
  ...
  return (
    <div className="p-8 space-y-10 animate-in fade-in duration-500 bg-slate-50 min-h-full overflow-y-auto custom-scrollbar">
      ...
      {totalProjects === 0 ? (
        <div className="bg-white rounded-[3rem] border-2 border-dashed border-slate-200 p-20 flex flex-col items-center text-center space-y-8 shadow-sm">
          <div className="w-24 h-24 bg-slate-50 rounded-[2rem] flex items-center justify-center text-slate-300">
            <FolderOpen className="w-12 h-12" />
          </div>
          <div className="space-y-3 max-w-xl">
            <h2 className="text-3xl font-black text-slate-900 tracking-tight">Zatiaľ nemáš vytvorený žiadny projekt</h2>
            <p className="text-slate-500 font-medium text-lg leading-relaxed">
              Začni vytvorením prvého projektu. BA Workspace bude následne zbierať požiadavky, rozhodnutia, otázky, riziká, deadliny, zdroje a AI výstupy okolo konkrétneho projektu.
            </p>
          </div>
          <div className="flex flex-col sm:flex-row items-center gap-4 pt-4">
            <button 
              onClick={() => navigate('/projects')}
              className="px-10 py-5 bg-indigo-600 text-white rounded-[1.5rem] font-black text-sm uppercase tracking-widest shadow-xl shadow-indigo-100 hover:bg-indigo-700 active:scale-95 transition-all flex items-center gap-3"
            >
              <Plus className="w-5 h-5" /> Vytvoriť prvý projekt
            </button>
            <button 
              onClick={() => {
                if (confirm('Naozaj chcete načítať demo dáta? Táto akcia pridá 3 testovacie projekty.')) {
                  loadDemoData();
                }
              }}
              className="px-10 py-5 bg-white border-2 border-slate-200 text-slate-600 rounded-[1.5rem] font-black text-sm uppercase tracking-widest hover:bg-slate-50 active:scale-95 transition-all flex items-center gap-3"
            >
              <Library className="w-5 h-5 text-indigo-500" /> Načítať demo dáta
            </button>
          </div>
        </div>
      ) : (
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {[
              ...
              { label: 'AVG Health Score', val: avgHealth !== null ? `${avgHealth}%` : 'N/A', icon: ShieldCheck, color: avgHealth && avgHealth > 75 ? 'text-emerald-600' : 'text-amber-600', bg: avgHealth && avgHealth > 75 ? 'bg-emerald-50' : 'bg-amber-50', border: avgHealth && avgHealth > 75 ? 'border-emerald-100' : 'border-amber-100' },
